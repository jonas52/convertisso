...
"13" "EXIT" 3>&1 1>&2 2>&3)
        if [ "$subtitle" = "1" ]                                     #vtt en srt     
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                    if [ "$?" = "0" ]; then srtt=$(find $FILE -name "*.srt")
                            if [ -n "$srtt" ]; then clear
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                    if [ "$?" = "0" ]; then asss=$(find $FILE -name "*.ass")
                            if [ -n "$asss" ]; then clear  
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                    if [ "$?" = "0" ]; then lrcc=$(find $FILE -name "*.lrc")
                            if [ -n "$lrcc" ]; then clear   
...
"43" "EXIT" 3>&1 1>&2 2>&3)
        if [ "$video" = "1" ]                                     #mkv en avi
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                    if [ "$?" = "0" ]; then mkvv=$(find $FILE -name "*.mkv")
                            if [ -n "$mkvv" ]; then clear
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                 if [ "$?" = "0" ]; then mp44=$(find $FILE -name "*.mp4")
                            if [ -n "$mp44" ]; then clear
                                echo "Conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                 if [ "$?" = "0" ]; then movv=$(find $FILE -name "*.mov")
                            if [ -n "$movv" ]; then clear
                                echo "Conversion in progress ..."                             
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                 if [ "$?" = "0" ]; then avii=$(find $FILE -name "*.avi")
                            if [ -n "$avii" ]; then clear
                                echo "Conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                    if [ "$?" = "0" ]; then webmm=$(find $FILE -name "*.webm")
                            if [ -n "$webmm" ]; then clear
                                echo "Conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                    if [ "$?" = "0" ]; then hevcc=$(find $FILE -name "*.hevc")
                            if [ -n "$hevcc" ]; then clear 
                                echo "Conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                    if [ "$?" = "0" ]; then flvv=$(find $FILE -name "*.flv")
                            if [ -n "$flvv" ]; then clear
                                echo "Conversion in progress ..." 
...
"50" "EXIT" 3>&1 1>&2 2>&3)
        if [ "$AUDIO" = "1" ]
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then wavv=$(find $FILE -name "*.wav")
                        if [ -n "$wavv" ]; then clear
                            echo "Conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then wavv=$(find $FILE -name "*.wav")
                        if [ -n "$wavv" ]; then clear
                            echo "conversion in progress ..."
                            for i in $wavv; do ffmpeg -i "$i" -map_metadata 0 -c:a aac "${i%.waw}.aac"> /dev/null 2>&1; done 
                            enco=aac
                            var=1
                        else
                            zenity --error --text="No compatible files found in the selected directory"
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then oggg=$(find $FILE -iname "*.ogg")
                        if [ -n "$oggg" ]; then clear
                            echo "conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then oggg=$(find $FILE -name "*.ogg")
                        if [ -n "$oggg" ]; then clear
                            echo "conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then oggg=$(find $FILE -name "*.ogg")
                        if [ -n "$oggg" ]; then clear
                            echo "conversion in progress ..."
                            for l in $oggg; do ffmpeg -i "$l" -map_metadata 0 -c:a aac "${l%.ogg}.aac"> /dev/null 2>&1; done 
                            enco=aac
                            var=1
                        else
                            zenity --error --text="No compatible files found in the selected directory"
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then ac33=$(find $FILE -name "*.ac3")
                        if [ -n "$ac33" ]; then clear
                            echo "conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then ac33=$(find $FILE -name "*.ac3")
                        if [ -n "$ac33" ]; then clear
                            echo "conversion in progress ..."
                            for o in $ac33; do ffmpeg -i "$o" -map_metadata 0 -c:a aac "${o%.ac3}.aac"> /dev/null 2>&1; done
                            enco=aac
                            var=1
                        else
                            zenity --error --text="No compatible files found in the selected directory"
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then aacc=$(find $FILE -name "*.aac")
                        if [ -n "$aacc" ]; then clear
                            echo "conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then flacc=$(find $FILE -name "*.flac")
                        if [ -n "$flacc" ]; then clear
                            echo "conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then opuss=$(find $FILE -name "*.opus")
                            if [ -n "$opuss" ]; then clear
                                echo "conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then opuss=$(find $FILE -name "*.opus")
                        if [ -n "$opuss" ]; then clear
                            echo "conversion in progress ..."
                            for tt in $opuss; do ffmpeg -i "$tt" -ab 320k -map_metadata 0 -c:a aac "${tt%.opus}.aac"> /dev/null 2>&1; done
                            enco=ac3 
                            var=1
                        else
                            zenity --error --text="No compatible files found in the selected directory"
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then vorbiss=$(find $FILE -name "*.vorbis")
                            if [ -n "$vorbiss" ]; then clear
                                echo "conversion in progress ..."
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
                if [ "$?" = "0" ]; then vorbiss=$(find $FILE -name "*.vorbis")
                        if [ -n "$vorbiss" ]; then clear
                            echo "conversion in progress ..."
                            for tt in $vorbiss; do ffmpeg -i "$tt" -ab 192k -map_metadata 0 -c:a aac "${tt%.vorbis}.aac"> /dev/null 2>&1; done
                            enco=ac3 
                            var=1
                        else
                            zenity --error --text="No compatible files found in the selected directory"
...
while [ $vor = 0 ]; do
        if [ "$image" = "1" ]                                      #png en jpg 
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
            if [ "$?" = "0" ]; then jpgg=$(find $FILE -name "*.jpg")
                        if [ -n "$jpgg" ]; then clear
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
            if [ "$?" = "0" ]; then tifff=$(find $FILE -name "*.tiff")
                        if [ -n "$tifff" ]; then clear
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
            if [ "$?" = "0" ]; then pdff=$(find $FILE -name "*.pdf")
                    if [ -n "$pdff" ]; then clear
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
            if [ "$?" = "0" ]; then svgg=$(find $FILE -name "*.svg")
                    if [ -n "$svgg" ]; then clear
            ...
            then FILE=$(zenity --file-selection --directory --title="Select one directory (not recusive)")
            if [ "$?" = "0" ]; then heicc=$(find $FILE -name "*.heic")
                    if [ -n "$heicc" ]; then clear
